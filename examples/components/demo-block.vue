...
import { stripScript, stripStyle, stripTemplate } from '../utils/index.js'
...
export default {
  ...
  computed: {
    ...
    langConfig () {
      // console.log(compoLang.filter(config => config.lang === this.lang)[0]['demo-block'])
      // return compoLang.filter(config => config.lang === this.lang)[0]['demo-block']
      return { 'hide-text': '隐藏代码', 'show-text': '显示代码', 'button-text': '在线运行', 'tooltip-text': '前往 codepen.io 运行此示例' }
    },
